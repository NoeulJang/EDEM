...
.button-wrapper a{
	            	...
	            	<div class="row">
	            		<div class="col-md-7 col-sm-7">
							<div class="form-group button-wrapper">
								<a href="/group/groupAddMember" class="btn btn-primary">가입 멤버 관리 보기</a>
 ...
 <%--            <c:if test="${groupMemberPage.hasNoGroupMember()}">
            ...
            <h4 class="grey">
				<i class="icon ion-android-checkmark-circle" style = "margin-left:5%">그룹 멤버 관리</i>
			</h4>
			 <c:if test="${memberAdminCount == 0}">
            	<p>그룹에 가입된 사람이 없습니다.</p>
            </c:if>
			</div>
...
$( document ).ready(function() {
	...
	$(window).scroll(function() {
		...
		if (scrTop >= docHi) {
			...
			page++;
			getPage("/group/groupMemberAdminList?page=" + page);
		}
	});
	...
	$('#show-list').on("click", ".friend-card", function(event) {
		...
		var join = confirm('회원을 추방하시겠습니까?');
			if (join == true) {
				$.ajax({
					type : 'get',
					url : '/group/removeMember/' + memno,
					headers : {
